...
class LostBus2Activity : AppCompatActivity() {
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val task = GetXMLTask()
        val api_key = "714a73676869797335366b6b454a4e"
        val url = "http://openapi.seoul.go.kr:8088/$api_key/xml/ListLostArticleService/1/200/b2/"
        task.execute(url)
    }
    ...
    private inner class GetXMLTask : AsyncTask<String?, Void?, Document?>() {
        ...
        protected override fun onPostExecute(doc: Document) {
            ...
            for (i in 0 until nodeList.length) { //데이터를 추출
                ...
                val get_positionList = fstElmnt.getElementsByTagName("GET_POSITION")
                // <GET_POSITION>습득위치_회사명</GET_POSITION>
                s.append("습득위치_회사명 = ").append(get_positionList.item(0).childNodes.item(0).nodeValue).append("\n")
